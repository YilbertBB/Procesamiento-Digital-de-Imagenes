...
class App(ctk.CTk):
    def __init__(self):
        ...
        # Top bar buttons
        self.topbar_button_open = ctk.CTkButton(
            self.topbar_frame, text='Open', command=self.open_image)
        self.topbar_button_open.grid(row=0, column=1, padx=10, pady=10)
        self.topbar_button_save = ctk.CTkButton(
            self.topbar_frame, text='Save', command=self.save_image)
        self.topbar_button_save.grid(row=0, column=2, padx=10, pady=10)
        self.topbar_button_revert = ctk.CTkButton(
            self.topbar_frame, text='Revert', command=self.revert_image)
        self.topbar_button_revert.grid(row=0, column=3, padx=10, pady=5)
        self.topbar_button_undo = ctk.CTkButton(
            self.topbar_frame, text='Undo', command=self.undo_image)
        ...
        self.scrollable_frame = ctk.CTkScrollableFrame(
            self, label_text="Options")
        self.scrollable_frame.grid(row=1, column=0, pady=0, sticky="nsew")
        self.scrollable_frame.grid_columnconfigure(0, weight=1)

        # Flip
        self.flip_label = ctk.CTkLabel(
            self.scrollable_frame, text="Flip", font=ctk.CTkFont(size=20, weight="normal"))
        self.flip_label.grid(row=0, column=0, padx=20, pady=(0, 5))
        self.sidebar_switch_flip_horizontal = ctk.CTkSwitch(
            self.scrollable_frame, text='Horizontal', command=lambda: self.flip_image('HORIZONTAL'))
        self.sidebar_switch_flip_horizontal.grid(row=1, column=0, padx=10)
        self.sidebar_switch_flip_vertical = ctk.CTkSwitch(
            self.scrollable_frame, text='Vertical', command=lambda: self.flip_image('VERTICAL'))
        self.sidebar_switch_flip_vertical.grid(row=2, column=0, padx=10)

        # Bright
        self.sidebar_label_bright = ctk.CTkLabel(
            self.scrollable_frame, text="Bright", font=ctk.CTkFont(size=20, weight="normal"))
        self.sidebar_label_bright.grid(row=3, column=0, padx=20, pady=(20, 5))
        self.sidebar_slider_bright = ctk.CTkSlider(
            self.scrollable_frame, command=self.brightness_filter)
        ...
        self.sidebar_label_contrast.grid(
            row=5, column=0, padx=20, pady=(20, 5))
        self.sidebar_slider_contrast = ctk.CTkSlider(
            self.scrollable_frame, command=self.contrast_filter)
        self.sidebar_slider_contrast.grid(row=6, column=0, padx=10)

        # Grayscale and negative filter
        self.sidebar_label_filter = ctk.CTkLabel(
            self.scrollable_frame, text="Filtros", font=ctk.CTkFont(size=20, weight="normal"))
        self.sidebar_label_filter.grid(row=7, column=0, padx=20, pady=(20, 5))
        self.sidebar_switch_grayscale = ctk.CTkSwitch(
            self.scrollable_frame, text='Grayscale', command=self.grayscale_filter)
        self.sidebar_switch_grayscale.grid(row=8, column=0, padx=10)
        self.sidebar_switch_negative = ctk.CTkSwitch(
            self.scrollable_frame, text='Negative', command=self.negative_filter)
        ...
        self.sidebar_button_rotate = ctk.CTkButton(
            self.scrollable_frame, text='Rotate', command=self.rotate_image)
        ...
        self.sidebar_button_move_horizontal = ctk.CTkButton(
            self.scrollable_frame, text='Horizontal', command=lambda: self.move_image('HORIZONTAL'))
        self.sidebar_button_move_horizontal.grid(
            row=13, column=0, padx=10, pady=5)
        self.sidebar_button_move_vertical = ctk.CTkButton(
            self.scrollable_frame, text='Vertical', command=lambda: self.move_image('VERTICAL'))
        self.sidebar_button_move_vertical.grid(
        ...
        # Suavizado
        self.sidebar_label_operations = ctk.CTkLabel(self.scrollable_frame, text="Suavizado",
                                                     font=ctk.CTkFont(size=20, weight="normal"))
        self.sidebar_label_operations.grid(
            row=20, column=0, padx=20, pady=(20, 5))
        self.sidebar_combobox_operations = ctk.CTkOptionMenu(self.scrollable_frame,
                                                             values=[
                                                                 'Paso Bajo', 'Promedio', 'Gaussiano', 'Mediana', 'Sobel', 'Laplaciano'],
                                                             command=self.suavizado,
                                                             state='readonly')
        self.sidebar_combobox_operations.grid(
        ...
        # Trasfomaciones morfologicas
        self.sidebar_label_operations = ctk.CTkLabel(self.scrollable_frame, text="Trasfomaciones \nMorfologiacas",
                                                     font=ctk.CTkFont(size=20, weight="normal"))
        self.sidebar_label_operations.grid(
            row=22, column=0, padx=20, pady=(20, 5))
        self.sidebar_combobox_operations = ctk.CTkOptionMenu(self.scrollable_frame,
                                                             values=[
                                                                 'Erosión', 'Dilatación', 'Apertura', 'Cierre'],
                                                             command=self.transformacionesM,
                                                             state='readonly')
        self.sidebar_combobox_operations.grid(
            row=23, column=0, padx=10, pady=5)
        ...
        self.sidebar_label_rotate = ctk.CTkLabel(self.scrollable_frame, text="Segmentacion",
                                                 font=ctk.CTkFont(size=20, weight="normal"))
        self.sidebar_label_rotate.grid(row=24, column=0, padx=20, pady=(20, 5))

        # Segmentacion Basasda en Umbralizacion
        self.sidebar_label_operations = ctk.CTkLabel(self.scrollable_frame, text="Basasda \nen\n Umbralizacion",
                                                     font=ctk.CTkFont(size=20, weight="normal"))
        self.sidebar_label_operations.grid(
            row=25, column=0, padx=20, pady=(20, 5))
        self.sidebar_combobox_operations = ctk.CTkOptionMenu(self.scrollable_frame,
                                                             values=[
                                                                 'Fija', 'Adaptativa', 'Binarización de Otsu'],
                                                             command=self.umbralizacion,
                                                             state='readonly')
        self.sidebar_combobox_operations.grid(
            row=26, column=0, padx=10, pady=5)
        ...
        # Segmentacion Basasda en componentes conexas
        self.sidebar_label_move = ctk.CTkLabel(self.scrollable_frame, text="Basasda \nen\n Componentes\n Conexas",
                                               font=ctk.CTkFont(size=20, weight="normal"))
        self.sidebar_label_move.grid(row=27, column=0, padx=20, pady=(20, 5))
        self.sidebar_switch_flip_horizontal = ctk.CTkSwitch(
            self.scrollable_frame, text='', command=self.connected_component_label)
        self.sidebar_switch_flip_horizontal.grid(row=28, column=0, padx=10)
    ...
    def open_image_ask(self):
        ...
        try:
            ...
            filename = filedialog.askopenfilename(
                initialdir='~/Pictures', title='Open image', filetypes=file_types)
            return None if filename == '' else filename
        except (OSError, FileNotFoundError):
            messagebox.showerror(
                'Error', 'Unable to find or open file <filename>')
    ...
    def save_image(self):
        ...
        if location is not None:
            try:
                ...
                messagebox.showerror(
                    'Error!', 'An error ocurred while saving the image!')

    def move_image(self, option):
        """Move an image"""
        try:
            amount = int(self.open_input_dialog_event(
                'Type the amount to move:', 'Move'))
            self.processor.move_image(option, amount)
            self.update_label_image()
        except:
            messagebox.showerror(
                'Error', 'The amount need to be a real number!')

    def resize_image(self, option):
        """Resize an image"""
        try:
            amount = int(self.open_input_dialog_event(
                'Type the amount to sum:', 'Resize'))
    ...
    def rotate_image(self):
        ...
        try:
            ...
            messagebox.showerror(
                'Error', 'The degrees need to be a real number!')
    ...
    def suavizado(self, option):
        """Aplies an smoothing into the image"""
        self.processor.suavizado(option)
        self.update_label_image()

    def transformacionesM(self, option):
        """Aplies an morphological transformations into the image"""
        self.processor.transformacionesM(option)
        self.update_label_image()

    def umbralizacion(self, option):
        """Aplies an thresholding into the image"""
        self.processor.umbralizacion(option)
        self.update_label_image()

    def connected_component_label(self):
        """Aplies an labeling of related components into the image"""
        self.processor.connected_component_label()
        self.update_label_image()
